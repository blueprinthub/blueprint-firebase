...
describe("OAuth2RepositoryContext", () => {
  ...
  describe("claimAccess", () => {
    it("should call JiraOAuthStrategy when platform = jira",
      async () => {
        const claimAccess = jest.spyOn(
          JiraOAuthStrategy.prototype, "claimAccess"
        ).mockImplementation();
        const getUser = jest.spyOn(
          JiraOAuthStrategy.prototype, "getUser"
        ).mockImplementation();

        const claim: OAuthClaim = {
          platform: "jira", code: "abc",
    ...
    it("should call AsanaOAuthStrategy when platform = asana",
      async () => {
        const claimAccess = jest.spyOn(
          AsanaOAuthStrategy.prototype, "claimAccess"
        ).mockImplementation();
        const getUser = jest.spyOn(
          AsanaOAuthStrategy.prototype, "getUser"
        ).mockImplementation();

        const claim: OAuthClaim = {
          platform: "asana", code: "abc",
          type: AuthenticatorType.Task,
        };
        const access: Omit<Access, "user"> = {
          accessToken: "test-token",
    ...
    it("should call GithubOAuthStrategy when platform = github", async () => {
      const claimAccess = jest.spyOn(
        GithubOAuthStrategy.prototype, "claimAccess"
      ).mockImplementation();
      const getUser = jest.spyOn(
        GithubOAuthStrategy.prototype, "getUser"
      ).mockImplementation();


      const claim: OAuthClaim = {
        platform: "github", code: "abc",
        type: AuthenticatorType.Task,
      };
      const access: Omit<Access, "user"> = {
        accessToken: "test-token",
