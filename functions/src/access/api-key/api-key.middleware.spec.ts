...
describe("ApiKeyCurrentUser middleware", () => {
  ...
  it("should send an error response for an invalid authorization header",
    async () => {
  ...
  it("should send an error response if the api key service returns null",
    async () => {
      const req = mockRequest({authorization: "Token " + VALID_API_KEY});
      const res = mockResponse();
  ...
  it("should send an error response if the token keyword is not 'Token'",
    async () => {
      const req = mockRequest({authorization: "Bearer " + VALID_API_KEY});
      const res = mockResponse();
  ...
  it("should send an error response if the api key is an empty string",
    async () => {
